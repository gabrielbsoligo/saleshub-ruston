...
export const PerfSdrView: React.FC = () => {
  ...
  const rankRows = Object.entries(bySdr).map(([id, v]) => {
    ...
    return { id, name: v.name, realizadas: v.realizadas, bant4: v.bant4, tarefas: t?.concluidas || 0, caixa: null as number | null };
  });
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6">
      ...
      {/* § TAREFAS */}
      <h3 className="text-sm font-semibold text-white mb-2 flex items-center gap-1.5"><ClipboardList size={14} className="text-[var(--color-v4-red)]" /> Tarefas concluídas</h3>
      <div className={`${card} mb-6`}>
        <div className="overflow-x-auto">
        <table className="w-full text-sm min-w-[380px]">
          <thead>
            <tr className="text-[11px] text-[var(--color-v4-text-muted)] text-left">
              <th className="px-2 py-1">SDR</th>
              <th className="px-2 py-1 text-right border-l border-[var(--color-v4-border)]">Concluídas</th>
              <th className="px-2 py-1 text-right border-l border-[var(--color-v4-border)]">Atrasadas (em aberto)</th>
            </tr>
          </thead>
          <tbody>
            {[...tar].sort((a, b) => (b.concluidas || 0) - (a.concluidas || 0)).map(t => (
              <tr key={t.member_id} className="border-t border-[var(--color-v4-border)] text-white">
                <td className="px-2 py-1.5">{t.name}</td>
                <td className="px-2 py-1.5 text-right font-semibold border-l border-[var(--color-v4-border)]">{t.concluidas}</td>
                <td className="px-2 py-1.5 text-right text-red-400 border-l border-[var(--color-v4-border)]">{t.atras_humano + t.atras_auto}</td>
              </tr>
            ))}
          </tbody>
        </table>
        </div>
        <div className="text-[10px] text-[var(--color-v4-text-muted)] mt-2 opacity-70">Concluídas <span className="text-white">a partir de 07/07</span> (limpeza anterior descartada pelo corte). Exclui conclusão <span className="text-white">em lote</span> (≥15 no mesmo minuto = varredura, não conta como esforço). Sem separar robô/humano — cumprir a cadência é esforço do SDR.</div>
      </div>
      ...
      <div className={`${card} mb-6`}>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {[
            ...
            { key: "tarefas", label: "Tarefas concluídas" },
