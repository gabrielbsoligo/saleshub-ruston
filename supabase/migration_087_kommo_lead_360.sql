-- migration_087_kommo_lead_360.sql
-- ORACULO Fatia 1: dossie consolidado do lead (get_lead_360) + helper de custom_fields.
-- ADITIVO/REVERSIVEL, SO LEITURA. Nao toca roleta/cadencia/anti-no-show/write.
-- Reverter:
--   DROP FUNCTION IF EXISTS public.kommo_lead_360(text);
--   DROP FUNCTION IF EXISTS kommo.lead_360(text);
--   DROP FUNCTION IF EXISTS kommo.resolve_custom_fields(jsonb);
--
-- Ponte de identidade (o que destontava o modelo): kommo.* usa id Kommo (bigint),
-- public.* usa UUID; ponte = public.leads.kommo_id = kommo.leads.id. A RPC resolve as
-- DUAS no inicio (kid bigint, uid uuid) e espalha pra todas as fontes.

-- ============================================================================
-- 1) HELPER: kommo.leads.custom_fields (jsonb ARRAY) -> objeto legivel.
--    Cada elemento ja traz field_name; select -> values[].value ja e o rotulo
--    (fallback: enum_id -> kommo.custom_fields.enums); date/date_time -> epoch -> BR.
-- ============================================================================
CREATE OR REPLACE FUNCTION kommo.resolve_custom_fields(p_cf jsonb)
RETURNS jsonb LANGUAGE sql STABLE AS $$
  SELECT COALESCE(jsonb_object_agg(fname, val), '{}'::jsonb)
  FROM (
    SELECT
      el->>'field_name' AS fname,
      CASE
        WHEN el->>'field_type' IN ('date','date_time','birthday')
             AND (el#>>'{values,0,value}') ~ '^[0-9]+$'
          THEN to_char(to_timestamp((el#>>'{values,0,value}')::bigint) AT TIME ZONE 'America/Sao_Paulo','DD/MM/YYYY HH24:MI')
        ELSE (
          SELECT string_agg(COALESCE(NULLIF(v->>'value',''), e.lbl), ' | ')
          FROM jsonb_array_elements(el->'values') v
          LEFT JOIN LATERAL (
            SELECT en->>'value' AS lbl
            FROM kommo.custom_fields cf, jsonb_array_elements(cf.enums) en
            WHERE cf.id = (el->>'field_id')::bigint AND (en->>'id') = (v->>'enum_id')
            LIMIT 1
          ) e ON true
        )
      END AS val
    FROM jsonb_array_elements(COALESCE(p_cf,'[]'::jsonb)) el
    WHERE el ? 'field_name'
  ) t
...
-- ============================================================================
-- 2) RPC: dossie consolidado. 2 visoes (por_tipo + timeline cronologica) + resumo.
-- ============================================================================
  ...
  SELECT jsonb_build_object(
    ...
    'por_tipo', jsonb_build_object(
      ...
      'ligacoes', (SELECT jsonb_agg(jsonb_build_object(
                ...
                FROM public.call_quality cq JOIN public.ligacoes_4com lg ON lg.call_id=cq.call_id
                WHERE cq.kommo_lead_id=kid),
      ...
      'tarefas', (SELECT jsonb_agg(jsonb_build_object(
                    ...
                    'concluida', tk.is_completed) ORDER BY tk.kommo_created_at)
  ...
  res := jsonb_set(res, '{timeline}',
    (SELECT jsonb_agg(e ORDER BY (e->>'ts') NULLS LAST) FROM (
       ...
       SELECT jsonb_build_object('ts',tk.kommo_created_at,'tipo','tarefa','ator',u3.name,
                'resumo',CASE WHEN tk.is_completed THEN '[feita] ' ELSE '[aberta] ' END||left(COALESCE(tk.text,''),120),
                'ref_id',tk.id::text)
...
-- ============================================================================
-- 3) WRAPPER public (padrao kommo_*): SECURITY DEFINER, so service_role.
-- ============================================================================
CREATE OR REPLACE FUNCTION public.kommo_lead_360(p_lead TEXT)
RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path=kommo,public AS
$$ SELECT kommo.lead_360(p_lead) $$;

DO $$ DECLARE f TEXT; BEGIN
  FOR f IN SELECT unnest(ARRAY['public.kommo_lead_360(text)'])
  LOOP
