...
const mapTask = (x: any) => ({ id: x.id, entity_type: x.entity_type, entity_id: x.entity_id, responsible_user_id: x.responsible_user_id, is_completed: x.is_completed, task_type_id: x.task_type_id, text: x.text, complete_till: tsISO(x.complete_till), kommo_created_at: tsISO(x.created_at), kommo_updated_at: tsISO(x.updated_at), result_text: x.result?.text ?? null })
