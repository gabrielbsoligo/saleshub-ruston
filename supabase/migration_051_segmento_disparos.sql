-- migration_051_segmento_disparos.sql
-- Campo "Segmento (disparos)" na importação de leads -> custom field do Kommo.
-- 1) coluna aditiva em public.leads (nullable, não quebra nada existente).
-- 2) kommo_post_create_lead passa a enviar o campo "Segmento Disparos" (id 1041897, texto)
--    quando preenchido. ADITIVO/REVERSÍVEL (drop column + recriar a função sem o bloco).

ALTER TABLE public.leads ADD COLUMN IF NOT EXISTS segmento_disparos TEXT;
...
BEGIN
    ...
    END IF;
    -- NOVO: Segmento Disparos (field_id 1041897, texto) — preenchido na importação
