-- migration_063_create_lead_mapa_canal.sql
-- CREATE path (kommo_post_create_lead): substitui o ramo ELSE (que mandava tudo
-- pro PV-Outbound velho 13250384) por um MAPA POR-CANAL pros funis NOVOS.
-- Canais no banco: minúsculos, sem acento (recovery/outbound/recomendacao/indicacao).
--
-- Mapa:
--   kommo_pipeline_id preenchido        -> usa ele (topo, inalterado)
--   blackbox / leadbroker (inbound)     -> Novo-Pré Vendas 14062096 / ENTRADA 108545092
--   recovery / outbound                 -> Novo-Outbound Disparo 14062116 / BASE 108545252
--   recomendacao / indicacao / QUALQUER -> Novo-Pré Vendas 14062096 / ENTRADA 108545092 (fallback, nunca velho)
--
-- Roleta 1 intacta (pipeline/status independe de sdr_id/responsible).
...
BEGIN
    ...
    -- MAPA POR-CANAL -> funis NOVOS (nunca cair em funil velho arquivado)
    ...
    ELSIF p.canal IN ('blackbox', 'leadbroker') THEN
        pipeline_id := 14062096; status_id := 108545092;   -- inbound -> Novo-Pré Vendas / ENTRADA
    ELSIF p.canal IN ('recovery', 'outbound') THEN
        pipeline_id := 14062116; status_id := 108545252;   -- Novo-Outbound Disparo / BASE
    ELSE
        pipeline_id := 14062096; status_id := 108545092;   -- recomendacao/indicacao/qualquer outro -> Novo-Pré Vendas / ENTRADA (fallback)
    END IF;
