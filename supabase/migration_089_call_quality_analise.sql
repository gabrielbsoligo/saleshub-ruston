-- migration_089_call_quality_analise.sql
-- Qualidade de Ligação: (1) vínculo do lead no reader; (2) expõe o `analise` inteiro pro modal.
-- ADITIVO/REVERSIVEL. Não toca roleta/cadência/anti-no-show.
-- Reverter: ALTER TABLE public.call_quality DROP COLUMN IF EXISTS analise;
--   (get_call_quality volta pela 077).

-- Coluna aditiva: guarda o objeto AVALIAÇÃO_DE_DESEMPENHO_DA_CALL inteiro (o edge passa a gravar).
ALTER TABLE public.call_quality ADD COLUMN IF NOT EXISTS analise jsonb;

-- get_call_quality: +lead_nome (resolve kommo_lead_id -> kommo.leads) e +analise
-- (fallback pro raw->'analise' das linhas já gravadas antes da coluna existir). Mesma assinatura.
...
RETURNS TABLE(
  ...
  kommo_lead_id bigint, lead_nome text, analise jsonb,
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public,kommo AS $$
  ...
  SELECT call_id, sdr_id, sdr_name, nota_final, pp, pn, transcricao, record_url, duration,
         direction, started_at, kommo_lead_id, lead_nome, analise, analisado_em, tem_analise,
         COUNT(*) OVER() AS total
  FROM base
  ORDER BY
