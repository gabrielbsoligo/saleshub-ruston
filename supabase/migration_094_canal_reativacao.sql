...
-- 1) CHECK da coluna canal: inclui 'reativacao'
...
ALTER TABLE public.leads ADD CONSTRAINT leads_canal_check
  CHECK (canal = ANY (ARRAY['blackbox','leadbroker','outbound','recomendacao','indicacao','recovery','reativacao']::text[]));

-- 2) sync p/ Kommo: 'reativacao' roteia e taggeia IGUAL ao 'recovery'
...
BEGIN
    ...
    ELSIF p.canal IN ('recovery', 'outbound', 'reativacao') THEN
        pipeline_id := 14062116; status_id := 108545252;   -- Novo-Outbound Disparo / BASE (reativacao = igual recovery)
    ELSE
