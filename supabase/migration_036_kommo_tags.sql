-- =============================================================
-- Migration 036 — Tags do Kommo na criação do lead
-- =============================================================
-- Permite escolher tags na importação; o lead sobe pro Kommo já com
-- elas (_embedded.tags). Tags inexistentes são criadas pelo Kommo.
-- =============================================================

ALTER TABLE leads ADD COLUMN IF NOT EXISTS kommo_tags TEXT[];
COMMENT ON COLUMN leads.kommo_tags IS 'Tags a aplicar no lead do Kommo na criação (_embedded.tags).';

-- Recria o trigger: agora monta _embedded com tags + contato.
...
BEGIN
    ...
    IF NEW.kommo_tags IS NOT NULL AND array_length(NEW.kommo_tags, 1) > 0 THEN
        SELECT jsonb_agg(jsonb_build_object('name', btrim(t)))
          INTO v_tags
          FROM unnest(NEW.kommo_tags) AS t
